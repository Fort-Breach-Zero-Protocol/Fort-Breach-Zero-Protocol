...
function showToast(message, type = 'info') {
    const colors = { success: '#22c55e', error: '#ef4444', warning: '#f97316', info: '#0ea5e9' };
    const background = colors[type] || colors.info;
    if (typeof Toastify === 'function') {
        Toastify({ text: message, duration: 3000, gravity: 'top', position: 'center', close: true, style: { background } }).showToast();
    } else {
    ...
    nextLevelBtn.on('pointerdown', async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                (window.top || window).location.replace('/home?levelCompleted=4');
            } else {
...
function spawnEnemyUnit(scene) {
    ...
    laneWeights[2] = 0.4; // 30% chance compared to normal lanes
    
    // Avoid portal lanes (lane 0 and lane 4) - portals will transform enemies to players
    laneWeights[0] = 0; // No enemies in lane 0 (portal lane)
    laneWeights[4] = 0; // No enemies in lane 4 (portal lane)
...
function onMeet(p, e) {
    ...
    // Merged players are immortal - they destroy the enemy but survive
    if (p.isMerged) {
        e.body.enable = false;
